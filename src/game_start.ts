class GameStart extends Phaser.State {
  ...
  preload() {
    //this.game.scale.scaleMode = (this.game.device.desktop)? Phaser.ScaleManager.SHOW_ALL : Phaser.ScaleManager.RESIZE;
  ...
  create() {
    ...
    playTXT.events.onInputDown.add(()=>{
          this.selectFX.play();
    ...
    instructionsTXT.events.onInputDown.add(()=>{
          instructionsTXT.visible = false;
          titleTXT.visible = false;
          playTXT.y = 600;
          this.selectFX.play();
          var instructionsExpTXT = this.game.add.bitmapText(0, 200, 'carrier_command','', 14);
          ...
          instructionsExpTXT.x = this.game.width / 2 - instructionsExpTXT.textWidth / 2;
